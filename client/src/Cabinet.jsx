...
const handleProfileUpdate = async () => {
  ...
  const surname = document.querySelector('input[placeholder="Фамилия"]').value;
...
const handleCancelOrder = async (orderId) => {
  ...
  try {
    ...
    const data = await response.json();
    if (data.success) {
      setOrders(prev => prev.map(o => o.id === orderId ? { ...o, order_status: 'отменен' } : o));
    } else {
      alert(data.message);
    }
  } catch (err) {
    ...
    <div className='cabinet-container'>
        <div className='cabinet-bg'><img src='/images/cabinet-bg.jpg' /></div>
        <div className='cabinet-elements'>
            <div className='cabinet-element'>
                <div className='cabinet-element-top' onClick={handleProfileEdit}><img src='/images/cabinet-edit-icon.png' /></div>
                <div className='cabinet-element-part'>
                    <div className='cabinet-photo'><img src='/images/cabinet-default-photo.png' /></div>
                ...
                <div className='cabinet-element-part'>
                    <div className='cabinet-element-part-data'>
                        ...
                        <div><h3>Телефон:</h3><p>{user.phone}</p></div>
                    </div>
                    <div className='cabinet-element-part-data'>
                        <img src='/images/cabinet-city-icon.png' />
                        <div><h3>Город:</h3><p>{user.city || 'Город не указан'}</p></div>
                    </div>
                </div>
                <div className='cabinet-element-part'>
                <button onClick={logout} style={{width: '100%', background: 'transparent', color: '#CA5538'}}>Выйти</button>
            </div>
            </div>
            <div className='cabinet-element' style={{background: 'rgb(72, 134, 185, 0.5)'}}>
                <div className='cabinet-element-part' style={{justifyContent: 'center'}}>
                    <div className='cabinet-image'><img src='/images/cabinet-partner-icon.png' /></div>
  ...
  {orders.length > 0 ? (
    orders.map(order => {
      let items = [];
      try {
        items = typeof order.order_info === 'string' ? JSON.parse(order.order_info).items : order.order_info.items;
      } catch (err) {
      ...
      return (
        <div className='cabinet-orders-list-order' key={order.id}>
          <div className='cabinet-orders-list-order-element'>
            ...
            <div>
              ...
              {items.length > 0 ? (
                items.map((item, index) => (
                  <p key={index} className='cabinet-orders-list-order-element-info'>
            ...
            <p style={{
  color: order.order_status === 'оплачен' ? 'green' :
         order.order_status === 'отменен' ? 'gray' :
         order.order_status === 'не оплачен' ? 'red' :
         'black'
}}>
  ...
  <button onClick={() => handleCancelOrder(order.id)}>Отменить</button>
    ...
    <p style={{color: '#fff'}}>Заказы отсутствуют</p>
        ...
        <div className='cabinet-edit-container' style={{display: isProfileEditActive ? 'flex' : 'none'}}>
            <div className='cabinet-edit'>
                <div className='cabinet-edit-photo'><img src='/images/cabinet-default-photo.png' /></div>
                ...
                <div className='cabinet-edit-citys'>
                    <input placeholder='Город' defaultValue={user.city} onClick={handleCitysList} />
                    <div className='cabinet-edit-citys-list' style={{display: isCitysListActive ? 'flex' : 'none'}}>
                        <div><p>Караганда</p></div>
                        <div><p>Астана</p></div>
                        <div><p>Алматы</p></div>
                    </div>
                </div>
                <button className='cabinet-edit-cancel-button' onClick={handleProfileEdit}>Отменить</button>
                <button className='cabinet-edit-confirm-button' onClick={handleProfileUpdate}>Подтвердить</button>
            </div>
        </div>
        <Navigation 
