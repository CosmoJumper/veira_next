...
const fetchProducts = useCallback(async (query, minPriceFilter, maxPriceFilter, criteria, category) => {
  setLoading(true);
  setError(null);
  try {
    ...
    const response = await fetch(`https://veira.kz/api/products-search?${params.toString()}`);
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Ошибка при загрузке продуктов: ${response.status} - ${errorText}`);
    }
  ...
  const shouldShowProducts = searchQuery.trim() !== '' || minPrice > min || maxPrice < max || selectedCriterions.length > 0 || selectedCategory !== '';

  // Загрузка продуктов при изменении поискового запроса, фильтров или категории
  useEffect(() => {
    if (shouldShowProducts) {
      debouncedFetchProducts(searchQuery, minPrice, maxPrice, selectedCriterions, selectedCategory);
    } else {
      setProducts([]);
    }
  }, [searchQuery, minPrice, maxPrice, selectedCriterions, selectedCategory, debouncedFetchProducts]);

  // Обработчик для применения фильтров
  const handleApplyFilters = () => {
    fetchProducts(searchQuery, minPrice, maxPrice, selectedCriterions, selectedCategory);
    setFilterOpen(false);
  ...
  return (
    ...
    >
      <div className="search-input-wrapper">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className='search-back-arrow' onClick={handleCloseSearch}>
          <path
            d="M15 18L9 12L15 6"
            stroke="#4886B9"
            strokeWidth="2"
        ...
        <div className="search-options" onClick={handleFilter}>
          <button><img src='/images/settings-icon.png' /></button>
        </div>
      </div>
  ...
  <Categories onCategorySelect={handleCategorySelect} />
      ...
      <div className='filter-container' style={{ display: isFilterOpen ? 'flex' : 'none' }}>
        <div className='filter-top'>
          <h1>Фильтры</h1>
          <button className='filter-button-close' onClick={handleFilter}>x</button>
        </div>
        ...
        <div className='filter-criterions-container'>
          <h2>Критерии:</h2>
          <div className='filter-criterions' style={{ height: criterionsIsOpen ? 'auto' : '100px', paddingBottom: criterionsIsOpen ? '30px' : '0' }}>
            {['Новинки', 'Хиты'].map(criterion => (
              <div
                key={criterion}
                className={`filter-criterion ${selectedCriterions.includes(criterion) ? 'selected' : ''}`}
        ...
        <div className='filter-bottom'>
          <button className='filter-button-clear' onClick={handleClearFilters}>Сбросить</button>
          <button className='filter-button-apply' onClick={handleApplyFilters}>Применить</button>
        </div>
      </div>
      <Navigation 
